const maps=[
  {id:'plains',name:'Terra-Prime // Széles kontinensfront',image:'assets/battlemap-plains-v31.png',text:'Nagyfelbontású kontinensfront falvakkal, folyókkal és stratégiai útvonalakkal.',stats:['3584×1800 HQ','FALVAK + IPAR','KÉT FŐVÁROS']},
...
const unitSpecs={
 infantry:{name:'Gyalogsági osztag',art:'assets/ember-infantry.png',badge:'GY',count:1000,cost:{food:50,metal:25},building:'barracks'},apc:{name:'Páncélozott szállító',art:'assets/ember-apc.png',badge:'PSZ',count:100,cost:{metal:140,oil:60,energy:30},building:'vehicleFactory'},tank:{name:'Harckocsi',art:'assets/ember-tank.png',badge:'HK',count:100,cost:{metal:240,oil:100,energy:65},building:'vehicleFactory'},heli:{name:'Támogató helikopter',art:'assets/ember-helicopter.png',badge:'LÉGI',count:100,cost:{metal:210,oil:160,energy:90},building:'hangar'},alien:{name:'Xeno portyázóraj',art:'assets/xenon-infantry.png',badge:'XENO',count:1000,cost:{food:50,metal:25},building:'barracks'},hover:{name:'Lebegő szállítóraj',art:'assets/xenon-apc.png',badge:'HOVER',count:100,cost:{metal:140,oil:60,energy:30},building:'vehicleFactory'},alienTank:{name:'Xeno ostromharckocsi',art:'assets/xenon-tank.png',badge:'X-TANK',count:100,cost:{metal:240,oil:100,energy:65},building:'vehicleFactory'},alienAir:{name:'Xeno légi elfogóraj',art:'assets/xenon-vtol.png',badge:'X-LÉGI',count:100,cost:{metal:210,oil:160,energy:90},building:'hangar'},strike:{name:'Csapásmérő vadászgép',art:'assets/ember-strike-fighter.png',badge:'CSAP',count:100,cost:{metal:290,oil:220,energy:130},building:'hangar'},alienStrike:{name:'Xeno csapásmérő',art:'assets/xenon-strike-fighter.png',badge:'X-CSAP',count:100,cost:{metal:290,oil:220,energy:130},building:'hangar'},mercenary:{name:'Semleges telepőr',art:'assets/neutral-warden-v1.svg',badge:'ŐR',count:120,cost:{},building:'barracks'},mercenaryCarrier:{name:'Civil terepjáró',art:'assets/neutral-carrier-v1.svg',badge:'CIV',count:1,cost:{},building:'vehicleFactory'}
};
...
const upgrades={commander:{name:'Parancsnoki hálózat',text:'Ennek a csapatnak +10% játékbeli erő.',cost:{tech:80,energy:100}},repair:{name:'Javító egység',text:'A járműves csapat morálja körönként gyorsabban töltődik.',cost:{metal:100,tech:90}},medics:{name:'Tábori kórház',text:'A csapat morálja körönként +8 ponttal gyorsabban töltődik.',cost:{food:120,tech:90}},artillery:{name:'Koordináció',text:'Ennek a csapatnak támadáskor +12% játékbeli erő.',cost:{metal:160,tech:130}},trenches:{name:'Védelmi állások',text:'Ennek a csapatnak védekezéskor +15% játékbeli erő.',cost:{metal:140,food:80}},logistics:{name:'Frontlogisztika',text:'Minden körben +5 energia és +5 étel.',cost:{metal:120,tech:80}},airSupport:{name:'Légi támogatás',text:'Légi egységet tartalmazó csapat +12% játékbeli erő.',cost:{oil:130,tech:150}}};
...
function emptyBase(){return {buildings:{barracks:false,vehicleFactory:false,hangar:false,researchCenter:false,depot:false},upgrades:{},queue:[],ready:{infantry:0,apc:0,tank:0,heli:0,alien:0,hover:0,alienTank:0,alienAir:0,strike:0,alienStrike:0},deploymentTarget:null}}
...
function unitFor(side,index,type=null){const human=['infantry','apc','tank','heli','strike'],alien=['alien','hover','alienTank','alienAir','alienStrike'],unitType=type||(side==='neutral'?'mercenary':(side==='human'?human:alien)[index%5]),spec=unitSpecs[unitType];return {side,type:unitType,morale:76,count:spec.count,composition:{[unitType]:spec.count},upgrades:{},lossDebt:{},movedTurn:null}}
...
function representativeType(unit){
  if(!unit)return 'infantry';
  const composition=unit.composition||{[unit.type]:unit.count};
  const priority=unit.side==='neutral'?['mercenaryCarrier','mercenary']:unit.side==='alien'?['hover','alienTank','alienAir','alienStrike','alien']:['apc','tank','heli','strike','infantry'];
  return priority.find(type=>(composition[type]||0)>0)||Object.keys(composition).find(type=>composition[type]>0)||unit.type;
}
function iconFor(unit){return unitSpecs[representativeType(unit)]?.badge||'ŐR'}
function artFor(unit){return unitSpecs[representativeType(unit)]?.art||'assets/infantry.png'}
function typeName(unit){return unitSpecs[representativeType(unit)]?.name||'Őrség'}
function hasMoved(unit){return unit?.movedTurn===state.turn}function ownBase(){return state.grid.find(s=>s.owner===state.faction&&s.isBase)}
...
function renderSectorModal(sector){
  ...
  const nonProductive=sector.resource==='terrain',village=sector.resource==='food'&&!sector.isBase,allowed=Object.entries(sectorBuilds).filter(([id,build])=>id==='outpost'||id==='bunker'||(!nonProductive&&!village&&build.key===sector.resource));
  const intro=village?'A falu minden nyersanyagból keveset termel. Csak őrtorony és bunker építhető itt.':nonProductive?'Nem termelő stratégiai terület. Itt csak felderítő őrtorony vagy egységtelepítésre szolgáló bunker építhető.':`${resourceLabel(sector.resource)} lelőhely. A helyi fejlesztések a kör végi termelést erősítik.`;
...
function applyCasualties(unit,loss){
  const composition=unit.composition||{[unit.type]:unit.count},entries=Object.entries(composition).filter(([,count])=>count>0);
  const total=entries.reduce((sum,[type,count])=>sum+count*unitWeight(type),0);
  if(!total)return 0;
  unit.lossDebt??={};unit.lastLosses={};let removed=0;
  for(const [type,count] of entries){
    const portion=loss*(count*unitWeight(type)/total)/unitWeight(type);
    const accumulated=portion+(unit.lossDebt[type]||0);
    /* A teljes alakulat részvételekor minden jelen lévő típus kapjon
       saját, egyszerű játékbeli veszteséget ugyanabban az ütközésben. */
    const amount=Math.min(count,Math.max(loss>=entries.length?1:0,Math.floor(accumulated)));
    unit.lossDebt[type]=Math.max(0,accumulated-amount);
    if(amount){composition[type]-=amount;unit.lastLosses[type]=amount;removed+=amount;if(!composition[type])delete composition[type]}
  }
...
async function endTurn(){
  if(state.aiInProgress||state.finished)return;
  state.turn++;state.selected=null;advanceQueues();
  Object.keys(state.resources).forEach(key=>state.resources[key]+=resourceIncome(key));
  for(const sector of state.grid){
    const unit=sector.unit;if(!unit||unit.side==='neutral')continue;
    const hasVehicles=Object.keys(unit.composition||{}).some(type=>['apc','hover','tank','alienTank','heli','alienAir','strike','alienStrike'].includes(type));
    const recovery=4+(sector.bunker?3:0)+(unit.upgrades?.medics?8:0)+(unit.upgrades?.repair&&hasVehicles?4:0);
    unit.morale=Math.min(100,unit.morale+recovery);
  }
  aiEconomy();renderBattle();await aiTurn();if(hasWon()){finishVictory();return}renderBattle();
}
...
function buildTerritoryHitModel(image){
  ...
  const width=source.width,height=source.height,size=width*height,wall=new Uint8Array(size),blockedTerrain=new Uint8Array(size),strongLine=new Uint8Array(size);
  for(let i=0;i<size;i++){const j=i*4,r=pixels[j],g=pixels[j+1],b=pixels[j+2];if(r>190&&g>178&&b>148&&r>=g&&g>=b&&r-g<55&&g-b<55)strongLine[i]=1}
...
function interiorAnchor(model,cell){
  const {width,height,region}=model,nearest={distance:Infinity,x:cell.x,y:cell.y};
  for(let y=Math.max(1,cell.minY+4);y<Math.min(height-4,cell.maxY);y+=4)
    for(let x=Math.max(1,cell.minX+4);x<Math.min(width-4,cell.maxX);x+=4){
      if(region[y*width+x]!==cell.id)continue;
      const margin=7;
      if(region[y*width+x-margin]!==cell.id||region[y*width+x+margin]!==cell.id||region[(y-margin)*width+x]!==cell.id||region[(y+margin)*width+x]!==cell.id)continue;
      const distance=(x-cell.x)**2+(y-cell.y)**2;
      if(distance<nearest.distance)Object.assign(nearest,{distance,x,y});
    }
  if(nearest.distance<Infinity)return nearest;
  for(let y=cell.minY;y<=cell.maxY;y+=3)for(let x=cell.minX;x<=cell.maxX;x+=3)
    if(region[y*width+x]===cell.id){const distance=(x-cell.x)**2+(y-cell.y)**2;if(distance<nearest.distance)Object.assign(nearest,{distance,x,y})}
  return nearest;
}
...
function combatPower(unit,side,defending=false){
  if(!unit)return 0;
  const training=unit.upgrades||{},base=unitStrength(unit)*(.38+.62*Math.max(0,unit.morale)/100);
  const leader=training.commander?1.1:1,coordination=training.artillery&&!defending?1.12:1;
  const defense=training.trenches&&defending?1.15:1;
  const air=(unit.composition?.heli||unit.composition?.alienAir||unit.composition?.strike||unit.composition?.alienStrike)&&training.airSupport?1.12:1;
  return base*leader*coordination*defense*air;
}
...
/* A bolygóőrök és a gazdaság absztrakt, könnyen olvasható játékértékei. */
function neutralUnitFor(resource,area,index){
  const productive=resource!=='terrain';
  const number=productive?Math.min(370,160+Math.round(area/135)+(index%4)*18):Math.min(140,48+Math.round(area/360)+(index%3)*12);
  const carrier=productive&&area>500&&index%4===0?1+(area>2400?1:0):0;
  const composition={mercenary:number};if(carrier)composition.mercenaryCarrier=carrier;
  return {side:'neutral',type:'mercenary',morale:75,count:number+carrier,composition,upgrades:{},lossDebt:{},movedTurn:null};
}
Object.values(sectorBuilds).filter(build=>build.bonus).forEach(build=>build.bonus=25);
function incomeFor(side,key){
  const yieldByResource={metal:32,energy:30,food:27,oil:32,tech:30};
  const owned=state.grid.filter(sector=>sector.owner===side);
  const sites=owned.reduce((sum,sector)=>sum+(sector.resource===key?(yieldByResource[key]||20):0),0);
  const villages=key==='food'?0:owned.filter(sector=>sector.resource==='food').length*3;
  const capital=owned.some(sector=>sector.isBase)?5:0;
  const built=side===state.faction?owned.reduce((sum,sector)=>sum+(state.sectorUpgrades[sector.id]||[]).reduce((bonus,id)=>bonus+(sectorBuilds[id]?.key===key?sectorBuilds[id].bonus:0),0),0):0;
  return 16+sites+villages+capital+built+(side===state.faction&&state.base?.upgrades.logistics&&['food','energy'].includes(key)?5:0);
}
function resourceIncome(key){return incomeFor(state.faction,key)}
function enemyResourceIncome(key){return incomeFor(enemyOf(state.faction),key)}
function upkeepFor(side){
  const units=state.grid.filter(sector=>sector.unit?.side===side).map(sector=>sector.unit);
  const infantry=units.reduce((sum,unit)=>sum+(unit.composition?.infantry||0)+(unit.composition?.alien||0),0);
  const vehicles=units.reduce((sum,unit)=>sum+Object.entries(unit.composition||{}).filter(([type])=>!['infantry','alien'].includes(type)).reduce((n,[,amount])=>n+amount,0),0);
  return Math.ceil(infantry/550+vehicles/18);
}
function netIncome(key){return resourceIncome(key)-(key==='food'?upkeepFor(state.faction):0)}
function payUpkeep(side){
  const cost=upkeepFor(side),stock=side===state.faction?state.resources:state.aiResources;
  if(!cost)return;
  const shortage=Math.max(0,cost-stock.food);stock.food=Math.max(0,stock.food-cost);
  if(shortage){state.grid.filter(sector=>sector.unit?.side===side).forEach(sector=>sector.unit.morale=Math.max(10,sector.unit.morale-3));addLog(`${side===state.faction?'Saját':'Ellenséges'} ellátmányhiány: a morál csökkent.`)}
}
function finishOutcome(victory){
  if(state.finished)return;
  state.finished=true;const layer=$('#victoryLayer');
  layer.classList.toggle('defeat',!victory);
  layer.querySelector('small').textContent=victory?'BOLYGÓ BIZTOSÍTVA':'A KOLÓNIA ELVESZETT';
  layer.querySelector('h2').textContent=victory?'VICTORY':'VERESÉG';
  layer.querySelector('p').textContent=victory?'A teljes pálya a tiéd.':'Az utolsó alakulatod is eltűnt a térképről.';
  layer.classList.add('open');layer.setAttribute('aria-hidden','false');
  setTimeout(()=>{layer.classList.remove('open');layer.setAttribute('aria-hidden','true');closeModal();show('menuView');state.finished=false},4000);
}
function finishVictory(){finishOutcome(true)}
function checkGameEnd(){
  ...
  if(state.grid.every(sector=>sector.owner===state.faction)){finishOutcome(true);return}
...
const renderBattleCore=renderBattle;
renderBattle=function(){renderBattleCore();checkGameEnd()};
async function endTurn(){
  if(state.aiInProgress||state.finished)return;
  state.turn++;state.selected=null;advanceQueues();
  Object.keys(state.resources).forEach(key=>state.resources[key]+=resourceIncome(key));
  payUpkeep(state.faction);
  state.grid.forEach(sector=>{const unit=sector.unit;if(!unit||unit.side==='neutral')return;
    const vehicle=Object.keys(unit.composition||{}).some(type=>['apc','hover','tank','alienTank','heli','alienAir','strike','alienStrike'].includes(type));
    unit.morale=Math.min(100,unit.morale+4+(sector.bunker?3:0)+(unit.upgrades?.medics?8:0)+(unit.upgrades?.repair&&vehicle?4:0));
  });
  aiEconomy();payUpkeep(enemyOf(state.faction));renderBattle();await aiTurn();checkGameEnd();renderBattle();
}

/* Egy csapat saját fejlesztései; a bázis kutatása csak a gazdaságé. */
const renderSquadInfo=renderSelection;
renderSelection=function(){
  renderSquadInfo();
  const sector=state.grid.find(item=>item.id===state.selected),unit=sector?.unit,el=$('#selectedUnit');
  if(!unit||unit.side!==state.faction||!el)return;
  const active=state.squadTab==='upgrades'?'upgrades':'details';
  const tabs=`<nav class="squad-tabs"><button class="${active==='details'?'active':''}" data-squad-tab="details">CSAPAT</button><button class="${active==='upgrades'?'active':''}" data-squad-tab="upgrades">✦ FEJLESZTÉS</button></nav>`;
  if(active==='details'){el.insertAdjacentHTML('afterbegin',tabs);return}
  const ready=Boolean(state.base?.buildings.researchCenter);
  const cards=Object.entries(upgrades).filter(([id])=>id!=='logistics').map(([id,up])=>{
    const done=Boolean(unit.upgrades?.[id]);
    return `<article class="squad-upgrade ${done?'done':''}"><b>${up.name}</b><p>${up.text}</p><small>${done?'AKTÍV':costText(up.cost)}</small><button type="button" data-squad-upgrade="${id}" ${done||!ready||!canAfford(up.cost)?'disabled':''}>${done?'✓ KÉSZ':!ready?'KUTATÓKÖZPONT KELL':'FEJLESZTÉS'}</button></article>`;
  }).join('');
  el.innerHTML=`${tabs}<b class="squad-upgrade-heading">${sector.name}</b><p class="squad-upgrade-intro">Ezek a fejlesztések csak erre az alakulatra érvényesek.</p>${cards}`;
};
function upgradeSquad(id){
  const sector=state.grid.find(item=>item.id===state.selected),unit=sector?.unit,up=upgrades[id];
  if(!unit||unit.side!==state.faction||!up||id==='logistics')return;
  if(!state.base?.buildings.researchCenter)return toast('Előbb építsd meg a kutatóközpontot.');
  if(unit.upgrades?.[id]||!pay(up.cost))return;
  unit.upgrades??={};unit.upgrades[id]=true;
  addLog(`${sector.name}: ${up.name} csak ennek a csapatnak aktív.`);renderBattle();
}
document.addEventListener('click',event=>{
  const tab=event.target.closest('[data-squad-tab]');if(tab){event.stopImmediatePropagation();state.squadTab=tab.dataset.squadTab;renderSelection();return}
  const upgrade=event.target.closest('[data-squad-upgrade]');if(upgrade){event.stopImmediatePropagation();upgradeSquad(upgrade.dataset.squadUpgrade);return}
  const nav=event.target.closest('[data-nav]');if(nav){
    event.stopImmediatePropagation();
    $$('.world-nav [data-nav]').forEach(button=>button.classList.toggle('active',button===nav));
    if(nav.dataset.nav==='upgrades'){state.squadTab='upgrades';renderSelection();$('.left-panel')?.scrollIntoView({behavior:'smooth',block:'nearest'})}
    else if(nav.dataset.nav==='squad'){state.squadTab='details';renderSelection();$('.left-panel')?.scrollIntoView({behavior:'smooth',block:'nearest'})}
    else if(nav.dataset.nav==='log')$('.right-panel')?.scrollIntoView({behavior:'smooth',block:'nearest'});
    else $('.map-wrap')?.scrollIntoView({behavior:'smooth',block:'nearest'});
  }
  const command=event.target.closest('[data-command="upgrade-squad"]');
  if(command){event.stopImmediatePropagation();state.squadTab='upgrades';renderSelection();if(!state.grid.find(item=>item.id===state.selected)?.unit)toast('Előbb válassz egy saját csapatot.');$('.left-panel')?.scrollIntoView({behavior:'smooth',block:'nearest'})}
},true);
...
function mergeDetachment(home,detachment){
  ...
  home.unit.count=total;home.unit.upgrades={...(home.unit.upgrades||{}),...(detachment.upgrades||{})};home.unit.morale=Math.min(100,Math.round((home.unit.morale*before+detachment.morale*detachment.count)/total));
}
...
function sectorBuild(id){
  ...
  if(sector.resource==='food'&&!sector.isBase&&!['outpost','bunker'].includes(id))return toast('Faluban csak őrtorony és bunker építhető.');
...
function mergeDetachment(home,detachment){if(!detachment?.count)return;if(!home.unit){home.unit=detachment;return}const before=home.unit.count,total=before+detachment.count;home.unit.composition={...(home.unit.composition||{})};Object.entries(detachment.composition||{[detachment.type]:detachment.count}).forEach(([type,count])=>home.unit.composition[type]=(home.unit.composition[type]||0)+count);home.unit.count=total;home.unit.upgrades={...(home.unit.upgrades||{}),...(detachment.upgrades||{})};home.unit.morale=Math.min(100,Math.round((home.unit.morale*before+detachment.morale*detachment.count)/total))}
function takeDetachment(unit,share){
  const source=unit.composition||{[unit.type]:unit.count},picked={};
  for(const [type,count] of Object.entries(source)){
    const take=share>=1?count:Math.max(count>0?1:0,Math.min(count,Math.round(count*share)));
    if(take>0)picked[type]=take;
    source[type]=count-take;if(!source[type])delete source[type];
  }
  unit.composition=source;unit.count=Object.values(source).reduce((sum,count)=>sum+count,0);
  const count=Object.values(picked).reduce((sum,value)=>sum+value,0);
  return {side:unit.side,type:unit.type,morale:unit.morale,count,composition:picked,upgrades:{...(unit.upgrades||{})},lossDebt:{},movedTurn:state.turn};
}
...
function buildTerritoryHitModel(image){
  ...
  for(let index=0;index<size;index++){
    ...
    const water=b>r*1.15&&b>g*1.03&&b>45;
    const northernCliffs=x>520&&x<1110&&y<165-55*Math.abs(x-815)/295;
    const southernCliffs=x>1000&&y>785+60*Math.abs(x-1370)/370;
    if(water||northernCliffs||southernCliffs){wall[index]=1;blockedTerrain[index]=1;continue}
    if(!territoryPixelIsBoundary(pixels,index*4))continue;
    let closeToWhite=false;
    for(let dy=-8;dy<=8&&!closeToWhite;dy+=2)for(let dx=-8;dx<=8;dx+=2){const px=x+dx,py=y+dy;if(px>=0&&py>=0&&px<width&&py<height&&strongLine[py*width+px]){closeToWhite=true;break}}
    if(!closeToWhite)continue;
  ...
  const valid=new Set(components.filter(item=>item.area>=450&&item.area<=72000).map(item=>item.id));
  ...
  cityOutlines.forEach(city=>{
    ...
    let area=0,sumX=0,sumY=0;for(let y=minY;y<=maxY;y++)for(let x=minX;x<=maxX;x++)if(inside(x+.5,y+.5,city.points)){
      region[y*width+x]=id;blockedTerrain[y*width+x]=0;area++;sumX+=x;sumY+=y;
    }
  ...
  /* A főváros kézzel követett határa kisajátíthatja egy szomszédos fehér
     körzet összes képpontját. Ilyen üres azonosítóhoz nem készül mező. */
  const actualArea=new Uint32Array(components.length);
  for(let i=0;i<region.length;i++)if(region[i]>=0&&valid.has(region[i]))actualArea[region[i]]++;
  for(const id of valid)if(actualArea[id]<400&&!Object.values(cityIds).includes(id))valid.delete(id);
  const adjacency=new Map([...valid].map(id=>[id,new Set()])),linkPoints=new Map();
  const connect=(a,b,x,y)=>{if(a===b||!valid.has(a)||!valid.has(b))return;adjacency.get(a).add(b);adjacency.get(b).add(a);if(Number.isFinite(x)&&Number.isFinite(y)){const key=[a,b].sort((m,n)=>m-n).join(':');const old=linkPoints.get(key);const i=(Math.round(y)*width+Math.round(x))*4,strong=pixels[i]>190&&pixels[i+1]>178&&pixels[i+2]>148?1:0;if(!old)linkPoints.set(key,{x,y,samples:1,strong});else{old.x+=x;old.y+=y;old.samples++;old.strong+=strong}}};
  ...
  for(let y=61;y<height-25;y+=3)for(let x=31;x<width-25;x+=3){
    ...
    const offset=center*4,r=pixels[offset],g=pixels[offset+1],b=pixels[offset+2];
    if(b>r*1.15&&b>g*1.03&&b>45){
      const bridges=[[360,170],[321,282],[410,530],[640,605],[705,712],[1275,348],[1316,396],[1412,460],[1460,533]];
      if(!bridges.some(([bx,by])=>(x-bx)**2+(y-by)**2<30**2))continue;
    }
    for(const d of [7,9,11,13]){
      connect(region[y*width+Math.max(0,x-d)],region[y*width+Math.min(width-1,x+d)],x,y);
      connect(region[Math.max(0,y-d)*width+x],region[Math.min(height-1,y+d)*width+x],x,y);
    }
  }
  ...
  /* A szigetek és a térkép szélén levő vízfoltok nem képeznek járható mezőt. */
  for(const id of [...valid])if(!Object.values(cityIds).includes(id)&&!adjacency.get(id)?.size){valid.delete(id);adjacency.delete(id)}
  for(const links of adjacency.values())for(const id of [...links])if(!valid.has(id))links.delete(id);
  return {width,height,region,blockedTerrain,areas,components,valid,adjacency,linkPoints,cityIds};
}
...
function createVisualTerritoryGrid(model){
  ...
  landmarks.forEach(([x,y,resource,name])=>{
    ...
    for(let dy=-30;dy<=30;dy+=2)for(let dx=-30;dx<=30;dx+=2){
      const px=x+dx,py=y+dy;if(px<0||py<0||px>=model.width||py>=model.height)continue;
      const id=model.region[py*model.width+px],distance=dx*dx+dy*dy;
      if(model.valid.has(id)&&id!==humanRegion&&id!==alienRegion&&(!best||distance<best.distance))best={id,distance};
    }
    if(best&&!landmarksByRegion.has(best.id))landmarksByRegion.set(best.id,{name,resource});
  });
  ...
  /* A két part között kizárólag a képen ténylegesen látható hidak kötnek össze
     körzeteket. Az irányjelző középpontja is magára a hídra kerül. */
  const bridgePairs=[
    [2363,942,250,369],[541,561,312,280],[214,141,363,172],
    [1423,1308,407,538],[1481,1419,650,605],[1620,1489,730,715],
    [197,122,1320,178],[683,741,1270,331],[933,1010,1306,393],
    [1228,1188,1417,456],[1228,1349,1464,532]
  ];
  const bridgeKeys=new Set(bridgePairs.map(([a,b])=>[a,b].sort((x,y)=>x-y).join(':')));
  const waterBetween=(a,b)=>{
    const x1=a.x*model.width/100,y1=a.y*model.height/100,x2=b.x*model.width/100,y2=b.y*model.height/100;
    const steps=Math.ceil(Math.hypot(x2-x1,y2-y1));let run=0,longest=0;
    for(let step=0;step<=steps;step++){
      const x=Math.max(0,Math.min(model.width-1,Math.round(x1+(x2-x1)*step/steps)));
      const y=Math.max(0,Math.min(model.height-1,Math.round(y1+(y2-y1)*step/steps)));
      run=model.blockedTerrain[y*model.width+x]?run+1:0;longest=Math.max(longest,run);
    }
    return longest>=20;
  };
  grid.forEach(sector=>{sector.links=sector.links.filter(id=>{
    const next=grid.find(item=>item.id===id),key=[sector.regionId,next?.regionId].sort((a,b)=>a-b).join(':');
    return next&&(bridgeKeys.has(key)||!waterBetween(sector,next));
  })});
  bridgePairs.forEach(([a,b,x,y])=>{
    const left=byRegion.get(a),right=byRegion.get(b);if(!left||!right)return;
    if(!left.links.includes(right.id))left.links.push(right.id);
    if(!right.links.includes(left.id))right.links.push(left.id);
    model.linkPoints.set([a,b].sort((n,m)=>n-m).join(':'),{x,y,samples:1,strong:1});
  });
  /* A híd nélküli, teljesen leválasztott parti szigetek nem kapnak
     kényszerített, a tengeren átvezető láthatatlan mozgási kapcsolatot. */
  for(let i=grid.length-1;i>=0;i--)if(!grid[i].links.length){model.valid.delete(grid[i].regionId);byRegion.delete(grid[i].regionId);grid.splice(i,1)}
  const available=new Map(grid.map(sector=>[sector.id,sector])),reachable=new Set(),pending=[grid.find(sector=>sector.owner==='human')?.id];
  while(pending.length){const id=pending.pop();if(!id||reachable.has(id))continue;reachable.add(id);pending.push(...(available.get(id)?.links||[]))}
  for(let i=grid.length-1;i>=0;i--)if(!reachable.has(grid[i].id)&&grid[i].owner==='neutral'){
    model.valid.delete(grid[i].regionId);byRegion.delete(grid[i].regionId);grid.splice(i,1);
  }
  grid.forEach(sector=>sector.links=sector.links.filter(id=>grid.some(other=>other.id===id)));
...
function paintTerritoryMap(canvas,model){
  ...
  const selected=state.grid.find(s=>s.id===state.selected),selectedRegion=selected?.regionId,hovered=state.hoveredRegion;
  const sectorByRegion=new Map(state.grid.map(s=>[s.regionId,s]));
  const palette={human:[31,134,240],alien:[152,53,196],humanAlly:[36,198,100],alienAlly:[235,65,68],neutral:[215,205,164]};
  const image=ctx.createImageData(model.width,model.height),px=image.data;
  const colour=s=>palette[s?.controlSide]||palette[s?.owner]||palette[s?.unit?.side]||palette.neutral;
  for(let i=0;i<model.region.length;i++){
    const id=model.region[i];if(!model.valid.has(id))continue;
    const sec=sectorByRegion.get(id),c=colour(sec);
    const side=sec?.controlSide||sec?.owner||sec?.unit?.side||'neutral';
    let a=side==='neutral'?0:(side==='human'?104:(side==='alien'?104:0));
    if(id===hovered)a=38;
    if(id===selectedRegion)a=112;
    const o=i*4;px[o]=c[0];px[o+1]=c[1];px[o+2]=c[2];px[o+3]=a;
  }
  /* Egyetlen képpontos, finom kontúr. Nincs vastag bézs háló. */
  const w=model.width,h=model.height,r=model.region;
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){
    const i=y*w+x,id=r[i];if(!model.valid.has(id))continue;
    const boundary=(model.valid.has(r[i+1])&&r[i+1]!==id)||(model.valid.has(r[i+w])&&r[i+w]!==id);
    if(!boundary)continue;
    const sec=sectorByRegion.get(id),c=colour(sec),o=i*4;
    const side=sec?.controlSide||sec?.owner||sec?.unit?.side||'neutral';
    px[o]=c[0];px[o+1]=c[1];px[o+2]=c[2];
    px[o+3]=side==='neutral'?58:105;
  }
...
function installTerritoryHitLayer(){
  ...
  const regionFromEvent=event=>{
    ...
    if(model.blockedTerrain[y*model.width+x])return -1;
    ...
    for(let radius=2;radius<=14;radius+=2)for(const [dx,dy] of [[radius,0],[-radius,0],[0,radius],[0,-radius],[radius,radius],[-radius,-radius]]){
  ...
  const bind=()=>{
    ...
    layer.addEventListener('pointermove',event=>{const id=regionFromEvent(event);if(state.hoveredRegion===id)return;state.hoveredRegion=model.valid.has(id)?id:null;const hovered=state.grid.find(item=>item.regionId===state.hoveredRegion),hint=map.querySelector('.territory-hover-hint');if(hint){hint.textContent=hovered?`${hovered.name} · ${resourceLabel(hovered.resource)}${hovered.unit?.side==='neutral'?` · Zsoldosok: ${hovered.unit.count}`:''}`:'';hint.style.display=hovered?'block':'none';if(hovered){hint.style.left=`${Math.min(84,Math.max(16,event.offsetX/layer.clientWidth*100))}%`;hint.style.top=`${Math.min(88,Math.max(12,event.offsetY/layer.clientHeight*100))}%`}}paintTerritoryMap(layer,model)});
    layer.addEventListener('pointerleave',()=>{state.hoveredRegion=null;const hint=map.querySelector('.territory-hover-hint');if(hint)hint.style.display='none';paintTerritoryMap(layer,model)});
...
function arrowMarkup(from,target,enemy=false){
  const model=state.territoryHitModel,pair=[from.regionId,target.regionId].sort((a,b)=>a-b).join(':'),border=model?.linkPoints?.get(pair);
  const x=border?border.x/border.samples/model.width*100:(from.x+target.x)/2,y=border?border.y/border.samples/model.height*100:(from.y+target.y)/2,deg=Math.atan2(target.y-from.y,target.x-from.x)*180/Math.PI,attack=target.unit?.side&&target.unit.side!==from.unit?.side;
...
function renderBattle(){
  const own=state.faction,enemy=enemyOf(own),selected=state.grid.find(sector=>sector.id===state.selected),map=maps.find(item=>item.id===state.map)||maps[0];
  $('.planet-scene').style.setProperty('--map-image',`url(${map.image})`);
  ...
  $('#resourceBar').innerHTML=Object.entries(state.resources).map(([key,value])=>`<div class="resource"><small>${key.toUpperCase()}</small><b>${value}</b><i>${netIncome(key)>=0?'+':''}${netIncome(key)}</i></div>`).join('');
  ...
  const markers=state.grid.map(sector=>{
    ...
    const guard=showUnit?(()=>{const neutral=sector.unit.side==='neutral',w=neutral?68:30,h=neutral?58:26,y=neutral?63:30;return `<span class="sector-guard unit-side-${sector.unit.side} type-${sector.unit.type} ${hasMoved(sector.unit)?'spent':''}" style="position:absolute!important;width:${w}px!important;height:${h}px!important;min-width:0!important;min-height:0!important;overflow:visible!important;transform:translateX(-50%)!important;z-index:90!important" title="${neutral?'Zsoldos őrség':typeName(sector.unit)} · ${sector.unit.count.toLocaleString('hu-HU')}"><span style="display:block!important;position:absolute!important;left:50%!important;top:0!important;width:${w}px!important;height:${h}px!important;min-width:${w}px!important;min-height:${h}px!important;max-width:${w}px!important;max-height:${h}px!important;transform:translateX(-50%)!important;background:url('${artFor(sector.unit)}') center/contain no-repeat!important;visibility:visible!important;opacity:1!important;z-index:91!important"></span><b style="display:block!important;position:absolute!important;left:50%!important;top:${y}px!important;transform:translateX(-50%)!important;visibility:visible!important;opacity:1!important;z-index:92!important">${sector.unit.count.toLocaleString('hu-HU')}</b>${hasMoved(sector.unit)?'<em>✓</em>':''}</span>`;})():'';
    ...
    const label=sector.isBase&&!active?'<span class="base-star" aria-hidden="true">★</span>':showLabel?`<span class="sector-label"><strong>${sector.name}${sector.isBase?' ★':''}</strong><small>${resourceLabel(sector.resource)}</small></span>`:sector.productive?`<span class="resource-pin" title="${sector.name} · ${resourceLabel(sector.resource)}">${{food:'●',metal:'◆',oil:'◉',tech:'✦',energy:'✧'}[sector.resource]||'●'}</span>`:'';
    ...
    return `<button class="territory-token ${sector.owner} ${active?'selected':''} ${sector.isBase?'base-sector':''} ${sector.unit?.composition?.mercenaryCarrier?'carrier-present':''}" style="left:${sector.x}%;top:${sector.y}%" data-sector="${sector.id}" aria-label="${sector.name}">${guard}${flashes}${installations}${label}</button>`;
  }).join('');
  ...
  $('#sectorMap').innerHTML=markers+arrows+aiArrow+moving+'<span class="territory-hover-hint" aria-live="off"></span>';installTerritoryHitLayer();renderSelection();
...
async function resolveDetachedConflict(home,target,attacker,aiMultiplier=1){
  ...
  const power=combatPower(attacker,attacker.side)*aiMultiplier,defense=combatPower(defender,defender.side,true),favored=power>=defense*.92;
  const defenderPoints=Math.max(1,Math.ceil(power*(favored?.22:.11))),attackerPoints=Math.max(1,Math.ceil(defense*(favored?.10:.18)));
  playAttackSound(attacker);const defenderLoss=applyCasualties(defender,defenderPoints),attackerLoss=applyCasualties(attacker,attackerPoints);defender.morale=Math.max(0,defender.morale-(favored?23:14));attacker.morale=Math.max(0,attacker.morale-(favored?10:22));flashCasualtyEntries([{sectorId:target.id,loss:defenderLoss,side:defender.side},{sectorId:home.id,loss:attackerLoss,side:attacker.side,kind:'retaliation'}]);renderBattle();await pause(1550);
  const won=defender.count===0||defender.morale===0;
  if(won&&attacker.count){if(window.vzBeforeCapture)await window.vzBeforeCapture(target,defender);target.unit=attacker;target.owner=attacker.side;if(attacker.side===state.faction)state.selected=target.id;addLog(`${target.name} védői feladták: az előrenyomuló alakulat elfoglalta a körzetet.`)}
