...
function renderBattle(){
  ...
  const markers=state.grid.map(sector=>{
    ...
    const guard=showUnit?`<span class="sector-guard unit-side-${sector.unit.side} type-${sector.unit.type} ${hasMoved(sector.unit)?'spent':''}" style="--unit-w:${sector.unit.side==='neutral'?68:63}px;--unit-h:${sector.unit.side==='neutral'?58:54}px;--count-y:${sector.unit.side==='neutral'?63:58}px" title="${sector.unit.side==='neutral'?'Zsoldos őrség':typeName(sector.unit)} · ${sector.unit.count.toLocaleString('hu-HU')}"><span class="unit-art" style="background-image:url('${artFor(sector.unit)}');width:${sector.unit.side==='neutral'?68:126}px!important;height:${sector.unit.side==='neutral'?58:108}px!important;position:absolute!important;left:50%!important;top:0!important;transform:translateX(-50%)!important;background-size:contain!important;background-repeat:no-repeat!important;background-position:center!important;display:block!important;visibility:visible!important;opacity:1!important;z-index:80!important"></span><b style="top:${sector.unit.side==='neutral'?63:112}px!important">${sector.unit.count.toLocaleString('hu-HU')}</b>${hasMoved(sector.unit)?'<em>✓</em>':''}</span>`:'';
