...
function renderBattle(){
  ...
  const markers=state.grid.map(sector=>{
    ...
    const guard=showUnit?`<span class="sector-guard ${sector.unit.side} type-${sector.unit.type} ${hasMoved(sector.unit)?'spent':''}" style="width:${sector.unit.side==='neutral'?78:132}px!important;height:${sector.unit.side==='neutral'?66:110}px!important;transform:translateX(-50%)!important" title="${sector.unit.side==='neutral'?'Zsoldos őrség':typeName(sector.unit)} · ${sector.unit.count.toLocaleString('hu-HU')}"><img src="${artFor(sector.unit)}" alt="" style="width:${sector.unit.side==='neutral'?78:132}px!important;height:${sector.unit.side==='neutral'?66:110}px!important;max-width:none!important;max-height:none!important;left:50%!important;top:0!important;transform:translateX(-50%)!important"/><b style="top:${sector.unit.side==='neutral'?71:115}px!important">${sector.unit.count.toLocaleString('hu-HU')}</b>${hasMoved(sector.unit)?'<em>✓</em>':''}</span>`:'';
