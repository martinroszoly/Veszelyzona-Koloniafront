...
function buildTerritoryHitModel(image){
  ...
  const connect=(a,b,x,y)=>{if(a===b||!valid.has(a)||!valid.has(b))return;adjacency.get(a).add(b);adjacency.get(b).add(a);if(Number.isFinite(x)&&Number.isFinite(y)){const key=[a,b].sort((m,n)=>m-n).join(':');const old=linkPoints.get(key);if(!old)linkPoints.set(key,{x,y,samples:1});else{old.x+=x;old.y+=y;old.samples++}}};
  ...
  for(let y=61;y<height-25;y+=3)for(let x=31;x<width-25;x+=3){
    ...
    if(b>r*1.16&&b>g*1.04&&g>36&&b>64){
  ...
  return {width,height,region,areas,components,valid,adjacency,linkPoints,cityIds};
}
...
function createVisualTerritoryGrid(model){
  ...
  const grid=cells.map((cell,index)=>{
    ...
    const anchor={x:inside.x/model.width*100,y:inside.y/model.height*100};
