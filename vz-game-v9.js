...
function createVisualTerritoryGrid(model){
  ...
  const grid=cells.map((cell,index)=>{
    const [generatedName,generatedResource]=territoryInfo(index),owner=cell.id===humanRegion?'human':cell.id===alienRegion?'alien':'neutral',isBase=owner!=='neutral';
    const anchor=isBase?(owner==='human'?{x:8.2,y:37.8}:{x:91.2,y:22.7}):{x:cell.x/model.width*100,y:cell.y/model.height*100};
    const resource=isBase?(owner==='human'?'energy':'tech'):generatedResource;
    const sector={id:`field-${cell.id}`,name:isBase?(owner==='human'?'Terra-Prime főváros':'Xeno dominion'):`${generatedName} ${index+1}`,x:anchor.x,y:anchor.y,w:(cell.maxX-cell.minX)/model.width*100,h:(cell.maxY-cell.minY)/model.height*100,resource,owner,isBase,productive:resource!=='terrain',bunker:isBase,watchtower:false,passable:true,terrain:'land',links:[],regionId:cell.id,unit:isBase?unitFor(owner,index,owner==='human'?'infantry':'alien'):unitFor('neutral',index,'mercenary')};
