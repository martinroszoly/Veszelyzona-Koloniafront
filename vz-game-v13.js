...
function unitFor(side,index,type=null){const human=['infantry','apc','tank','heli'],alien=['alien','hover','alienTank','alienAir'],unitType=type||(side==='neutral'?'mercenary':(side==='human'?human:alien)[index%4]),spec=unitSpecs[unitType];return {side,type:unitType,morale:76,count:spec.count,composition:{[unitType]:spec.count},upgrades:{},lossDebt:{},movedTurn:null}}
...
function representativeType(unit){
  ...
  const priority=unit.side==='alien'?['hover','alienTank','alienAir','alien']:['apc','tank','heli','infantry'];
...
const unitWeights={infantry:1,alien:1,mercenary:1,apc:85,hover:85,tank:155,alienTank:155,heli:115,alienAir:115};
...
async function endTurn(){
  ...
  for(const sector of state.grid){
    ...
    const hasVehicles=Object.keys(unit.composition||{}).some(type=>['apc','hover','tank','alienTank','heli','alienAir'].includes(type));
...
function combatPower(unit,side,defending=false){
  ...
  const air=(unit.composition?.heli||unit.composition?.alienAir)&&training.airSupport?1.12:1;
...
function createVisualTerritoryGrid(model){
  ...
  grid.filter(sector=>!sector.links.length).forEach(sector=>{const near=grid.filter(other=>other!==sector&&Math.hypot(other.x-sector.x,other.y-sector.y)<5).sort((a,b)=>Math.hypot(a.x-sector.x,a.y-sector.y)-Math.hypot(b.x-sector.x,b.y-sector.y))[0];if(near){sector.links=[near.id];if(!near.links.includes(sector.id))near.links.push(sector.id)}});
...
function paintTerritoryMap(canvas,model){
  ...
  for(let index=0;index<model.region.length;index++){
    ...
    if(regionId===selectedRegion)colour=owner==='alien'?[190,72,218,165]:owner==='human'?[36,142,246,165]:[255,205,70,126];
    else if(regionId===hovered)colour=[255,238,166,58];
    else if(owner==='human')colour=[31,134,240,91];
    else if(owner==='alien')colour=[152,53,196,94];
...
function renderBattle(){
  ...
  const markers=state.grid.map(sector=>{
    ...
    const label=showLabel?`<span class="sector-label"><strong>${sector.name}${sector.isBase?' ★':''}</strong><small>${resourceLabel(sector.resource)}</small></span>`:sector.productive?`<span class="resource-pin" title="${sector.name} · ${resourceLabel(sector.resource)}">${{food:'●',metal:'◆',oil:'◉',tech:'✦',energy:'✧'}[sector.resource]||'●'}</span>`:'';
