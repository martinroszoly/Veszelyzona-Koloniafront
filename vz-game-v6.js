...
function territoryPixelIsBoundary(data,offset){
  ...
  return r>170&&g>170&&b>145&&high-low<90;
}
function buildTerritoryHitModel(image){
  ...
  const region=new Int32Array(size);region.fill(-1);const areas=[],queue=new Int32Array(size);let regionId=0;
  for(let start=0;start<size;start++){
    if(wall[start]||region[start]!==-1)continue;
    let head=0,tail=0,area=0;queue[tail++]=start;region[start]=regionId;
    while(head<tail){
      const current=queue[head++],x=current%width,y=(current/width)|0;area++;
    ...
    areas.push(area);regionId++;
  }
  const valid=new Set(areas.map((area,id)=>area>900?id:-1).filter(id=>id>=0));
  return {width,height,region,areas,valid};
}
function nearestTerritoryForPixel(x,y,model){
  let best=null,bestDistance=Infinity;
  for(const sector of state.grid){
    const dx=x-sector.x/100*model.width,dy=y-sector.y/100*model.height,distance=dx*dx+dy*dy;
    if(distance<bestDistance){bestDistance=distance;best=sector;}
  }
  return best;
}
...
function installTerritoryHitLayer(){
  ...
  const ready=()=>{draw();layer.addEventListener('click',event=>{const rect=layer.getBoundingClientRect(),model=state.territoryHitModel;if(!model)return;const x=Math.max(0,Math.min(model.width-1,Math.floor((event.clientX-rect.left)/rect.width*model.width))),y=Math.max(0,Math.min(model.height-1,Math.floor((event.clientY-rect.top)/rect.height*model.height))),regionId=model.region[y*model.width+x];
    if(regionId<0||!model.valid.has(regionId))return;const sector=nearestTerritoryForPixel(x,y,model);if(!sector)return;state.territoryRegionBySector??={};state.territoryRegionBySector[sector.id]=regionId;selectSector(sector.id);event.stopPropagation();
  });};
