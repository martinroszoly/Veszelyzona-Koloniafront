const maps=[
  {id:'plains',name:'Terra-Prime // Széles kontinensfront',image:'assets/terra-prime-ultrawide-v4.png',text:'Kétszer szélesebb hadszíntér, kisebb falvakkal és ritkán elhelyezett stratégiai pontokkal. A terep és az utak maradnak előtérben.',stats:['150+ STRATÉGIAI PONT','FALVAK + IPAR','KÉT FŐVÁROS']},
  {id:'snow',name:'Terra-Prime // Jégfront',image:'assets/terra-prime-snow-v5.png',text:'Ugyanez a kiterjedt stratégiai elrendezés, hóval fedett utakkal és befagyott tavakkal.',stats:['97 JÁRHATÓ MEZŐ','HÓ + JÉGVIDÉK','KÉT FŐVÁROS']}
];
...
const sectorBuilds={mine:{name:'Bánya',icon:'⛏',key:'metal',bonus:16,cost:{metal:80,energy:45}},refinery:{name:'Finomító',icon:'◉',key:'oil',bonus:16,cost:{metal:95,energy:60}},generator:{name:'Generátor',icon:'ϟ',key:'energy',bonus:16,cost:{metal:90,oil:35}},farm:{name:'Élelmiszer-dóm',icon:'◌',key:'food',bonus:16,cost:{metal:75,energy:45}},outpost:{name:'Őrtorony',icon:'⌂',key:'guard',bonus:0,cost:{metal:125,energy:70}},bunker:{name:'Bunker',icon:'▣',key:'bunker',bonus:0,cost:{metal:160,energy:90,food:45}}};
...
function renderSectorModal(sector){
  ...
  const nonProductive=sector.resource==='terrain',allowed=Object.entries(sectorBuilds).filter(([id])=>!nonProductive||id==='outpost'||id==='bunker');
...
function hasWon(){const enemy=enemyOf(state.faction);return !state.grid.some(sector=>sector.owner===enemy||sector.unit?.side===enemy)}
...
function renderBattle(){
  ...
  const sectors=state.grid.map(s=>{
    ...
    if(!visible)return '';
    const guard=s.unit&&visible?`<span class="sector-guard ${s.unit.side} type-${s.unit.type} ${hasMoved(s.unit)?'spent':''}" data-select-unit="${s.id}" title="${typeName(s.unit)}"><img src="${artFor(s.unit)}" alt="${typeName(s.unit)}"/><i>${iconFor(s.unit)}</i><b>${s.unit.count.toLocaleString('hu-HU')}</b>${hasMoved(s.unit)?'<em>✓</em>':''}</span>`:'';
    ...
    const label=visible?`<strong>${s.name}${s.isBase?' ★':''}</strong><small>${resourceLabel(s.resource)}${s.unit?.side==='neutral'?' · ZS':''}</small>`:'<strong>FELDERÍTETLEN</strong><small>?</small>';
    ...
    const width=(s.w||3.8)*1.72,height=(s.h||8.3)*1.55;
    return `<button class="world-sector sector-shape-${s.shape||0} ${owner} terrain-${s.resource} terrain-kind-${s.terrain||'land'} ${active?'selected':''} ${s.isBase?'base-sector':''} ${hasMoved(s.unit)?'unit-spent':''}" style="left:${s.x}%;top:${s.y}%;width:${width}%!important;height:${height}%!important;clip-path:${sectorOutline(s.shape)}!important" data-sector="${s.id}" aria-label="${visible?s.name:'Felderítetlen szektor'}">${guard}${flashes}${installations}<span class="sector-label">${label}</span></button>`;
  }).join('');
  ...
  $('#sectorMap').innerHTML=sectors+arrows+aiArrow+moving;renderSelection();
...
function sectorBuild(id){
  ...
  if(sector.resource==='terrain'&&!['outpost','bunker'].includes(id))return toast('Nem termelő területen csak őrtorony vagy bunker építhető.');
